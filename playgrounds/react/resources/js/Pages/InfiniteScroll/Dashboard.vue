...
<template>
  ...
  <div class="flex space-x-12">
    <div class="relative mt-6 h-96 max-w-2xl flex-1 overflow-auto rounded-lg border-4 border-gray-200">
    ...
    <div class="mt-6 h-96 max-w-2xl flex-1 overflow-auto rounded-lg border-4 border-gray-200">
      <table class="w-full text-left">
        <thead>
          <tr class="sticky top-0 bg-white shadow">
          ...
          <InfiniteScroll prop="users" preserve-url>
            <template v-if="users">
              <tr v-for="user in users.data" :key="user.id" class="border-b">
                <td class="px-4 py-2 uppercase text-teal-600">{{ user.name }}</td>
              </tr>
            </template>

            <template #fetching>
              <tr>
                <td class="bg-gray-200 px-4 py-2 text-center" colspan="2">Fetching more users...</td>
              </tr>
            </template>
          </InfiniteScroll>
        </tbody>
